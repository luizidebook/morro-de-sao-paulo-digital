...
function recoverNavigationIfNeeded() {
  ...
  // Em recoverNavigationIfNeeded()
  // Adicionar mais verificações e recuperações
  if (!ensureNavigationComponents()) {
    console.warn(
      "[recoverNavigationIfNeeded] Falha ao garantir componentes de navegação"
    );
  }

  // Verificar se o marcador tem o evento correto de atualização
  if (window.userMarker && typeof updateUserMarkerDirection === "function") {
    try {
      updateUserMarkerDirection(
        window.userLocation,
      ...
      console.warn(
        "[recoverNavigationIfNeeded] Erro ao atualizar direção do marcador:",
        markerError
      );
    }
  }
...
/**
 * Calcula o progresso atual da navegação na rota
 * @param {Object} currentPosition - Posição atual do usuário
 * @param {Array} instructions - Array de instruções da rota
 * @param {number} currentStepIndex - Índice atual na lista de instruções
 * @returns {Object} - Objeto contendo percentual de progresso, distância completada e total
 */
function calculateRouteProgress(
  ...
  try {
    ...
    let totalDistance = 0; // CORREÇÃO: Definir totalDistance aqui
    ...
    if (
      ...
      console.warn("[calculateRouteProgress] Parâmetros inválidos:", {
        position: currentPosition,
        hasInstructions: Array.isArray(instructions),
        count: Array.isArray(instructions) ? instructions.length : 0,
      });
      return {
        percentage: 0,
        completed: 0,
        total: 100, // Valor padrão para evitar divisão por zero
      };
    }

    // 1. Calcular a distância total da rota somando todas as etapas
    totalDistance = instructions.reduce((sum, instruction) => {
      return sum + (instruction.distance || 0);
    }, 0);

    // 2. Calcular a distância já percorrida
    // (soma das etapas anteriores + parte da etapa atual)
    for (let i = 0; i < currentStepIndex; i++) {
      completedDistance += instructions[i].distance || 0;
    }

    // 3. Estimar a parte da etapa atual que já foi percorrida
    if (
      ...
      const currentStep = instructions[currentStepIndex];
      const stepStart = currentStep.maneuver.location;

      // Converter para formato padrão se necessário
      const stepLat = Array.isArray(stepStart)
        ? stepStart[1]
        : stepStart.lat || stepStart.latitude;

      const stepLon = Array.isArray(stepStart)
        ? stepStart[0]
        : stepStart.lon || stepStart.lng || stepStart.longitude;

      // Se tivermos coordenadas válidas para o início do passo
      if (stepLat && stepLon) {
        // Calcular distância da posição atual até o início do passo
        const distanceToStepStart = calculateDistance(
          currentPosition.latitude || currentPosition.lat,
          currentPosition.longitude ||
            currentPosition.lon ||
            currentPosition.lng,
          stepLat,
        ...
        // Estimar quanto da etapa atual já foi percorrido
        // (baseado na proporção da distância)
        const currentStepDistance = currentStep.distance || 0;
        if (currentStepDistance > 0) {
          // Calcular proporção percorrida (limitado entre 0-100%)
          const proportion = Math.max(
            0,
            Math.min(1, 1 - distanceToStepStart / currentStepDistance)
          );
          completedDistance += proportion * currentStepDistance;
        }
      } else {
        // Se não temos dados precisos, estimamos que estamos no meio do passo
        completedDistance += (currentStep.distance || 0) / 2;
      }
    }

    // Garantir que totalDistance seja sempre maior que zero para evitar divisão por zero
    totalDistance = Math.max(totalDistance, 1);
    ...
    if (isNaN(percentage)) {
      percentage = 0;
      console.warn("[calculateRouteProgress] Percentual inválido, usando 0%");
    }
...
function updateNavigationProgress(userLocation) {
  try {
    ...
    // Na função updateRealTimeNavigation
    ...
    // Verificar se progress é um objeto válido antes de usar
...
export function updateRealTimeNavigation(userPos = null) {
  // Adicione no início da função updateRealTimeNavigation
  console.log("[updateRealTimeNavigation] Estado atual:", {
    isActive: navigationState.isActive,
    hasInstructions: Array.isArray(navigationState.instructions),
    instructionCount: navigationState.instructions?.length || 0,
  ...
  if (navigationState.lastProcessedPosition) {
    const MOVEMENT_THRESHOLD = 1.5; // Metros (reduzido para maior sensibilidade)
    ...
    if (
      ...
      timeSinceLastUpdate < FORCE_UPDATE_INTERVAL / 2 // Ajustar para mais frequente
    ) {
      console.log(
...
export function updateUserMarkerDirection(userPos, routePoints) {
  ...
  try {
    ...
    // Em updateUserMarkerDirection()
    // Melhorar a lógica para encontrar pontos adequados na rota
    let nextPointIndex = nearestPointIndex;
    let nextPoint = null;
    const MIN_DISTANCE_FOR_DIRECTION = 5; // Reduzir para 5m para mais precisão

    // Procurar um ponto à frente que seja significativo
    for (
      let i = nearestPointIndex + 1;
      i < Math.min(routePoints.length, nearestPointIndex + 30);
      i++
    ) {
      // Aumentar o número de pontos inspecionados
      const point = routePoints[i];
      const lat = Array.isArray(point) ? point[0] : point.lat;
      const lon = Array.isArray(point) ? point[1] : point.lng || point.lon;
      ...
      if (lat !== undefined && lon !== undefined) {
        ...
        // Verificar se o ponto está a uma distância adequada
        if (distanceToPoint > MIN_DISTANCE_FOR_DIRECTION) {
          nextPointIndex = i;
          nextPoint = { lat, lon };
          break;
        }
...
function estimateRemainingTime(distance, speed = 1.4) {
  // Garantir que a distância seja válida e pelo menos 1 metro
  if (!distance || isNaN(distance) || distance <= 0) {
    return 0;
  }

  // Garantir uma velocidade mínima para evitar tempos infinitos
  const validSpeed = Math.max(0.5, speed);

  // Calcular o tempo baseado na distância e velocidade
  const timeInSeconds = Math.round(distance / validSpeed);

  // Aplicar um fator de ajuste para considerar paradas, curvas, etc.
  const adjustmentFactor = 1.2; // 20% adicional

  // Aplicar um limite máximo de 2 horas para evitar valores absurdos
  return Math.min(7200, Math.round(timeInSeconds * adjustmentFactor));
}
